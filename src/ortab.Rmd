...
orfunc <- function(efref, htval, pint = FALSE) {
  ...
  dummymod <- glm(formula(paste0(
    "ht == '", htval, "' ~ ", paste(modvars$var, collapse = " + ")
  )),
  data = pdata
  )
  ...
  for (i in seq_along(modvars$var)) {
    amod <- hf_glm_mids(formula(paste0(
      "ht == '", htval, "' ~ ", modvars$var[i], " * relevel(shf_ef_cat, ref = '", efref, "') + ", paste(modvars$var[modvars$var != modvars$var[i]], collapse = " + ")
    )),
    data = imp, subset = quote(ht %in% c("Normal", htval))
    )
...
```{r forestorref, fig.cap="Forest Adjusted OR - HFrEF", dependson=c("ortab", "forestor"), cache=cacheon}

orforestfunc(forestdata = bind_cols(orrefnr[, c(1, 3:5)], orrefr[, 3:5]))
```

```{r forestormref, fig.cap="Forest Adjusted OR - HFmrEF", dependson=c("ortab", "forestor"), cache=cacheon}

orforestfunc(forestdata = bind_cols(ormrefnr[, c(1, 3:5)], ormrefr[, 3:5]))
```

```{r forestorpef, fig.cap="Forest Adjusted OR - HFpEF", dependson=c("ortab", "forestor"), cache=cacheon}
