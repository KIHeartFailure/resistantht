...
survfunc <- function(time, event, eventname) {
  ...
  colnames(out) <- c(
    "",
    paste(rep(levsef, each = 3), rep(c(levels(pdata$ht)), 3)), "p-value interaction"
  )
  ...
  for (i in seq_along(levsef)) {
    ...
    mod <- coxph(formula(paste0(
      "Surv(", time, ",", event, "=='Yes') ~ ht * relevel(shf_ef_cat, ref = '", levsef[i], "')"
    )),
    data = pdata
    )
    smod <- summary(mod)
...
colnames(coxall) <- c("", rep(c(levels(pdata$ht)), 3), "p-value interaction")
