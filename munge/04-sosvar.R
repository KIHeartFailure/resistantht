...
patreg2 <- left_join(pdata %>% select(LopNr, shf_indexdtm),
  ...
  mutate(
    sos_com_durationhypertension = as.numeric(difftime(shf_indexdtm, INDATUM,
      units = "days"
    )),
    tmp_ht = stringr::str_detect(DIA_all, " I1[0-5]")
  ) %>%
  dplyr::filter(
    sos_com_durationhypertension >= 0,
...
pdata <- left_join(pdata,
  ...
  mutate(
    sos_com_durationhypertension_cat = factor(case_when(
      sos_com_durationhypertension > 5 * 365.25 ~ 4,
    ...
    sos_com_hypertension = case_when(
      sos_com_durationhypertension_cat == "No" ~ "No",
      TRUE ~ "Yes"
    ) # overwrite existing ht variables since only 5 years back
  )
